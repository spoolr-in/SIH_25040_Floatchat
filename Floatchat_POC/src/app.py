...
def display_results(results_df, fig, entities, query_text):
    ...
    with st.spinner("🧠 Generating intelligent summary..."):
        ...
        if summary_text:
            st.markdown(f"""
            <div class="stats-box" style="background-color: #e8f4fd; border-color: #1f77b4;">
                <div class="stats-title" style="color: #0d47a1;">{'AI-Enhanced' if use_llm else 'Quick'} Summary</div>
                <div style="color: #424242; line-height: 1.6; font-size: 1rem;">
                    {summary_text}
                </div>
            </div>
            """, unsafe_allow_html=True)
