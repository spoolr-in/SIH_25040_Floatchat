...
class DataSummarizer:
    ...
    def _generate_rule_based_summary(self, df: pd.DataFrame, entities, stats: Dict = None) -> str:
        """Generate summary using rule-based analysis with natural language"""
        
        summary_parts = []
        
        # Query context - more conversational
        query_context = self._get_query_context_natural(entities, len(df))
        summary_parts.append(query_context)
        
        # Data coverage analysis
        coverage_analysis = self._analyze_data_coverage_natural(df, entities)
        summary_parts.append(coverage_analysis)
...
Provide a 4-5 sentence analysis covering:
1. Key findings and what they indicate about current ocean conditions
2. Comparison to normal/expected values and any anomalies detected
3. Implications for maritime operations, environmental policy, or climate monitoring
4. Data reliability and coverage (time period, sensor network, geographic scope)
5. Actionable insights for decision-making

Write professionally but avoid excessive technical jargon. Focus on practical implications."""
